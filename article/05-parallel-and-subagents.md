...
> **版本说明：** 本文保留的是最初的极简同步子 agent 实现，便于讲清基本原理。当前代码已在同一 `TaskController` 下加入 `TaskSpec`/`TaskResult`、权限与路径边界、预算/超时/取消、受限重试与熔断、Git Worktree 后端，以及分阶段 Agent Team；以 `corecoder/delegation.py` 和 `corecoder/workspaces.py` 为准。
...
Claude Code 的子 agent 系统（它的 AgentTool 公开拆解里超过一千行）要丰富得多。CoreCoder 现在用更小的中央协议实现了关键执行形态：有界的前台或后台任务、隔离 Git Worktree、分阶段角色模板，以及仅父 Agent 可用的任务控制；递归仍被明确截断在一层。
