...
from corecoder.delegation import TaskResult, TaskStatus
...
async def test_spawn_researcher():
    ...
    try:
        ...
        assert " error:" not in result.casefold(), result
    finally:
        agent.close()
        llm.close()
...
async def test_agent_tool_with_role():
    ...
    try:
        ...
        print(result[:600])
        parsed = TaskResult.model_validate(json.loads(result))
        assert parsed.status == TaskStatus.COMPLETED, parsed.error or parsed.summary
        assert parsed.requires_parent_review
    finally:
        agent.close()
        llm.close()
