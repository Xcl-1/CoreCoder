...
**Sub-agents now run behind one parent-owned control plane.** The model-facing API has no `single`/`coding_team` switch: the main agent handles simple work itself and emits zero, one, or several `agent` calls according to the independent sub-tasks it finds. Calls from one response run concurrently within a hard limit; dependent implementation and review are scheduled only after prerequisite results return. Every delegation is a validated `TaskSpec`: objective, minimal context, exact tool allowlist, read/write roots, token and tool-call budgets, timeout, role, workspace mode, and acceptance criteria. `TaskController` owns the state machine, concurrency cap, timeout and cancellation. A child has an independent history, cannot receive the `agent` tool, cannot ask for permission elevation, and returns a bounded `TaskResult`; runtime code—not the child model—derives status, changed files, usage and policy violations. The parent still performs final review and acceptance.

Relative file/search paths and shell commands are anchored to each Agent's `workspace_root`. Delegated read/write roots are resolved before execution and must remain inside the parent workspace, including after canonical path resolution; out-of-workspace scopes are rejected before the child model runs.
...
Both execution backends use the same protocol. `fork` operates in the shared tree. The main agent may delegate up to four children from one model response and the controller runs up to three concurrently by default; use `CORECODER_MAX_SUBAGENTS_PER_ROUND` and `CORECODER_TASK_CONCURRENCY` (both 1–32, with concurrency no greater than the per-round limit) to adjust the bounds. `worktree` requires a clean Git parent, runs the child in a detached managed worktree, collects a binary diff, checks it centrally with `git apply --check`, applies it to the parent, and records the result in `/undo`; a conflict leaves the parent untouched and retains the isolated tree for inspection. `bash` and `undo_changes` have no reliably enforceable path argument and are rejected in delegated specs unless a library caller explicitly opts into unscoped tools; worktree tasks reject them unconditionally.

The model no longer selects a fixed team mode. It explicitly delegates the smallest useful set of researcher, executor, or reviewer tasks, so simple requests use no children and independent investigations can fan out dynamically. `Agent.run_team()` remains available only as a programmatic library template when an application deliberately wants a fixed staged pipeline. Read-only failures may opt into up to three budget-sharing retries, and repeated failures open a controller circuit breaker before more child work is admitted.
